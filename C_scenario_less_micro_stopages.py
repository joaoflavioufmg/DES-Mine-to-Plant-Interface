...
ajuste_OPtime = 0  #acrescenta minutos no tempo de operação
ajuste_OPtimeMina = 0
...
def distribuicoes (tipo):
  return {
      ...
      #-------------------------
      #silo 1 manutenção
      #'manutencao_no_silo1' : lognormvariate(5.3688 , 0.96541),    #Delay de manutenção no silo1 5.3560,0.9942
      'manutencao_no_silo1' : lognormvariate(5.2683 , 1.0337),   #sem 50% microparadas
      #-------------------------
      #silo 1 operação
      #'operacao_no_silo1' : lognormvariate(6.9813,0.3952),                  #MTBF original
      #'operacao_no_silo1' : weibullvariate(1289.2133 , 12.1283),  #MTBF Full
      #'operacao_no_silo1' : weibullvariate(329.8336 , 1.5309),  #MTBF Full por intervalos de operação
      'operacao_no_silo1' : weibullvariate(1302.2373 , 12.4994),  #MTBF sem 50% das microparadas   
      #-------------------------
      # silo 2 manutenção
      #'manutencao_no_silo2' : lognormvariate(5.493 , 0.86121),                #Delay de manutenção no silo2 5.4930,0.8612
      'manutencao_no_silo2' : lognormvariate(5.3753 , 0.94011),              #sem 50% microparadas 
      #-------------------------
      #silo 2 operação
      #'operacao_no_silo2' : lognormvariate(6.9348,0.5244),                  #Tempo de operação do silo2 MTBF
      #'operacao_no_silo2' : weibullvariate(1286.8478 , 14.3821), #MTBF Full
      #'operacao_no_silo2' : weibullvariate(417.5202 , 1.8041), #MTBF Full por intervalos de operação
      'operacao_no_silo2' : weibullvariate(1286.909 , 12.1577), #MTBF sem 50% das microparadas 
      #-------------------------
      #'manutencao_no_silo1' : lognormvariate(0.15332 , 1.3459),            #sem agregação diária
      #'operacao_no_silo1' : lognormvariate(1.7193 , 1.8715),                  #sem agregação diária
      #'manutencao_no_silo2' : lognormvariate(0.55406 , 1.2891),            #sem agregação diária    
      #'operacao_no_silo2' : lognormvariate(2.0104 , 1.7363),                  #sem agregação diária
...
def carrega_na_mina(env, id_caminhao, caminhoes, origem, trator_mina,trator_estoque_mina, silo1_acesso, silo2_acesso):
  ...
  if operando_mina == "":
    while True:
        operacao_mina = distribuicoes('operacao_na_mina') + ajuste_OPtimeMina  #Distribuição de operação entre falha na mina
        if operacao_mina > 14:
            break
    ...
    tempo_total_operando_mina += operacao_mina
      ...
      if env.now - inicio_de_operacao_mina >= operacao_mina:                    #Verifico se o tempo de operação entre falha foi atingido
        ...
        tempo_parada = distribuicoes('manutencao_na_mina') 
        ...
        while True:
            operacao_mina = distribuicoes('operacao_na_mina') + ajuste_OPtimeMina  
            if operacao_mina > 14:
                break
        tempo_total_operando_mina+=operacao_mina
...
def descarrega_nos_silos(env, id_caminhao, caminhoes, origem, trator_mina, trator_estoque_mina, silo1_acesso, silo2_acesso):
  ...
  if tamFilaSilo1 <= tamFilaSilo2:                                              #Verifico qual menor fila para descarregamento nos silos
    ...
    if operando_silo1 == "":
      while True:
        operacao_silo1 = distribuicoes('operacao_no_silo1') + ajuste_OPtime                     #Distribuição de operação entre falha do silo1
        if operacao_silo1>0:
            break
      ...
      if env.now - inicio_de_operacao_silo1 >= operacao_silo1:                  #Verifico se o tempo de operação entre falha foi atingido
        ...
        while True:
          tempo_parada = distribuicoes('manutencao_no_silo1')
          if tempo_parada>0:
            break
        ...
        while True:
          operacao_silo1 = distribuicoes('operacao_no_silo1') + ajuste_OPtime
          if operacao_silo1>0:
            break
        tempo_total_operando_britagem+=operacao_silo1
    ...
    if operando_silo2 == "":
      while True:
        operacao_silo2 = distribuicoes('operacao_no_silo2') + ajuste_OPtime                     #Distribuição de operação entre falha do silo2
        if operacao_silo2>0:
            break
      ...
      if env.now - inicio_de_operacao_silo2 >= operacao_silo2:                  #Verifico se o tempo de operação entre falha foi atingido
        ...
        while True:
          tempo_parada = distribuicoes('manutencao_no_silo2')
          if tempo_parada>0:
            break
        tempo_total_parada_britagem+=tempo_parada
        yield env.timeout(tempo_parada)                 #Delay de manutenção no silo2
        ...
        while True:
          operacao_silo2 = distribuicoes('operacao_no_silo2') + ajuste_OPtime
          if operacao_silo2>0:
            break
        tempo_total_operando_britagem+=operacao_silo2
